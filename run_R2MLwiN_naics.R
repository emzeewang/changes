...
data <- read_rds("data_for_analysis_naics.rds") %>%
  arrange(., year) 
...
for (x in x.vec) {
  ...
  for (i in 1:num.iter) {
    ...
    data.window <- mutate(data.window, year_gvkey = paste0(year, gvkey), gvkey_ind = paste0(gvkey, NAICSS1), year_ind = paste0(year, NAICSS1)) %>%
    ...
    model <- runMLwiN(performance_d ~ 1 + (1 | NAICSS1) + (1 | gvkey) + (1 | gvkey_ind) + (1 | ID), 
